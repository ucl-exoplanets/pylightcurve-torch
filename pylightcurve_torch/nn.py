"""This module defines the class TransitModule to model exoplanets transits in Pytorch."""
...
class TransitModule(nn.Module):
    """Pytorch module to compute primary and secondary transits of
    exoplanets."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        The model computes kepler positions, primary and secondary transits flux_drop drops for N
        different sets of parameters and T time steps.

         Paameters:
        -------
        :param time: array-like time series of time values. Shape: (T,), (1, T), (N, T)
        :param primary: boolean indicating whether to compute the primary transit or not (type:
            bool ; default: True)
        :param secondary: Whether to compute the secondary transit or not (type: bool ; default:
            False)
        :param epoch_type: str indicating whether the t0 parameter refers to mid primary or 
            secondary transit times. It can take the str values 'primary' or 'secondary'. If the
            primary param is set to True, it will be defaulted to 'primary', and otherwise to 
            'secondary'.
        :param precision: numerical precision between 1 and 6 (type: int ; default: 3)
        :param dtype: tensors dtype (default: torch.float64) If None provided, the environment
            default torch dtype will be used.
        :param cache_pos: whether or not to save the position tensors for efficiency. Note that
            this requires it to be outside of the Dynamic Computational Graph, and will be set to
            False with a warning as soon as a dependable parameter has its gradient activated.
        :param cache_flux: whether or not to save the flux decrements tensors for efficiency. Note
            that this requires it to be outside of the Dynamic Computational Graph, and will be set
            to False with a warning as soon as a dependent parameter has its gradient activated.
        :param cache_dur: whether or not to save the duration tensors for efficiency. Note that 
            this requires it to be outside of the Dynamic Computational Graph, and will be set to
            False with a warning as soon as a dependable parameter has its gradient activated.
        :param kwargs: additional optional parameters. If given these must be authorised transit
            parameters, as listed below:
            #Transit parameters
            -------
            rp: ratio of planetary by stellar radii  - unitless - alias: rp_ove_rs
            fp: ratio of planetary by stellar fluxes - unitless - alias: fp_over_rs (specific to 
                secondary transit)
            ...
            method: str indicating the limb-darkening law (available methods: 'claret', 'quad', 
                'sqrt' or 'linear') (specific to primary transit)
            ldc: A list containing the limb darkening coefficients. The list should contain 1 
                element if the method used is 'linear', 2 if the method used is 'quad' or 'sqrt',
                and 4 if the method used is 'claret'. (specific to primary transit) - alias:
    ...
    def __repr__(self):
        ...
        return (
            f"TransitModule({'primary=True, ' if self.primary else ''}"
            + f"{'secondary=True, ' if self.secondary else ''}shape={self.shape})"
        )

    def forward(self, **kwargs):
        """Return the combined flux drop of primary/secondary transits relative
        to the star.

        Overrides torch.nn.Module.forward method, itself aliased by
        __call__
    ...
    def set_param(self, name, value):
        ...
        Parameter will be defined as a class Attribute, most
        specifically a nn.Parameter except for 'method' which is just a
        str.
    ...
    def set_params(self, **kwargs):
        ...
        Parameters are accessible as class attributes. Except method,
        all params are torch tensor parameters
    ...
    def set_time(self, data, time_unit=None):
        ...
        :param data: array-like time series of time values. Shape: (T,)
            or (N, T)
        ...
        if self.shape[0] in [None, 1]:
            ...
            raise RuntimeError(
                "incompatible batch dimensions between data and parameters"
    ...
    def activate_grad(self, *args):
        ...
        This will also deactivate and reset the cache for the dependent
        tensors, with a possible warning if applicable.
    ...
    def reset_params(self, *args):
        ...
        :param args: list of parameters names. If None is provided, all
    ...
    def reset_cache(self, name=None, deactivate=False):
        ...
        Cached tensors are reset to None value, selectively if a
        parameter name is given.
        :param name: parameter name from which to selectively infer the
            dependent cached tensors. If no name provided ('default'),
            all the cached tensors will be reset (to None).
        :return:
        """
        if name is None:
            self.__pos = None
    ...
    def get_ldc_dim(self, data=None):
        ...
        Consistency with the method attribute is important and should be
        checked beforehand whenever possible. :data: optional tensor
        from which to infer the dimensionality
        ...
        if self.method is not None:
            ...
            if len(data.shape) > 1:
                ...
                warnings.warn(
                    "Neither of method nor ldc shape seem to provide a clear ldc dimensionality."
    ...
    def get_input_params(self, function="none", prepare_args=True, allow_none=False, **kwargs):
        ...
        :param function: which function to provide the params of - 'none', 'position', 'duration', 
            'drop_p', 'drop_s'
        :param prepare_args: whether to prepare or not the external arguments to correct type and 
            shape
        ...
        for k in kwargs:
            ...
            if not parname in self._parnames:
                raise RuntimeError(f"parameter {parname} not in authorized model's list")
    ...
    def get_position(self, **kwargs):
        """Compute the cached or computed 3D cartesian positions of the planet
        following a Keplerian orbit.

        if any model parameter is provided while calling this method, no
        caching will take place

        :param kwargs: optional external parameters to be provided as
            key/values pairs and to replace module's pars
        ...
        x, y, z = exoplanet_orbit(
            ...
            d["t0"],
            d["time"],
            ww=d["time"].new_zeros(1, 1, dtype=self.dtype),
            n_pars=batch_size,
            dtype=self.dtype,
        )
    ...
    def get_proj_dist(self, orbit_type=None, **kwargs):
        ...
        Distances are normalised with respect to the stellar radius. and
        projected onto the viewing plane.
        :param orbit_type: if set to 'primary' or 'secondary', will set
            a high float constant outside of the concerned transit times
        :param kwargs: optional external parameters to be provided as
            key/values pairs and to replace module's pars
        ...
        if orbit_type is None:
            ...
            return torch.where(
                x < 0.0, torch.ones_like(x, dtype=self.dtype) * MAX_RATIO_RADII, proj_dist
            )
    ...
    def get_drop_p(self, **kwargs):
        """Return the cached or computed flux_drop drop of transit(s), relative
        to the star.

        :param precision: precision of computation (int between 1 and 6)
        :param kwargs: additional functions argument to replace the
            class' one, disabling caching where necessary
        ...
        out = _transit_flux_drop(
            d["method"], d["ldc"], d["rp"], proj_dist, precision=self.precision, n_pars=batch_size
        )
    ...
    def get_drop_s(self, **kwargs):
        """Return the cached or computed flux_drop drop of eclipse(s), relative
        to the star.
        ...
        out = _transit_flux_drop(
            "linear",
            torch.zeros(batch_size, 1, dtype=d["rp"].dtype, device=d["rp"].device),
            1 / d["rp"],
            proj_dist,
            precision=self.precision,
            n_pars=batch_size,
        )
        out = (1.0 + d["fp"] * out) / (1.0 + d["fp"])
        if self.cache_flux and not runtime_mode:
            self.__drop_s = out
        return out
    ...
    def shape(self):
        ...
        :return: tuple of dimensions (N, T) where N is the batch size
    ...
    def _prepare_value(self, name, value, update_shape=True):
        ...
        if update_shape:
            if self.shape[0] in [None, 1]:
                ...
                raise RuntimeError(
                    "incompatible batch dimensions between parameters"
    ...
    def _check_method(self, value):
        ...
        ldc parameters attribute will be reset if inconsistent with the
        new method value.
        :param value: one of [None, 'linear', 'sqrt', 'quad', 'claret']
        :return:
        """
        ...
        if self.ldc is not None and self.ldc.shape[-1] != self._methods_dim[value]:
            warnings.warn(
                f"ldc method ({value}) incompatible with ldc tensor dimension "
                + f"({self.ldc.shape[-1]}). Resetting ldc coefs.")
